...
<!-- Slides -->
    ...
    <div class="carousel-item d-flex {{if {index} == 1 }}active{{end-if}}" style="background-image: url('{{slider.image.getImage()}}');" alt="{{slider.alt_text}}">
      <div class="carousel-container">
        <h1 class="animated fadeInDown text-white" style="background:transparent">{{slider.header_text}}</h2>
        <p class="animated fadeInUp"style="background:transparent">{{slider.body_text}}</p>
       {{ if {slider.button_text} != '' }} <a href="#about" {{if {slider.button_new_tab} == 1 }} target="blank" {{end-if}} class="btn btn-info animated fadeInUp scrollto">{{slider.button_text}}</a> {{end-if}}
      </div>
    </div>
    {{end-each}}
<!-- End Slides -->
    ...
    <a class="carousel-control-prev" href="#heroCarousel" role="button" data-slide="prev">
      <span class="carousel-control-prev-icon bx bx-chevron-left" aria-hidden="false"></span>
      <span class="sr-only sr-only-focusable">Previous</span>
    </a>

    <a class="carousel-control-next" href="#heroCarousel" role="button" data-slide="next">
      <span class="carousel-control-next-icon bx bx-chevron-right" aria-hidden="false"></span>
      ...
      <div class="row">
        ...
        <div class="col-lg d-flex align-items-stretch mb-5 mb-lg-0" data-aos="fade-up">
          <div class="icon-box d-flex flex-column align-items-center justify-content-between w-100">
            ...
            <h2 class="title"><a href="">{{card.title}}</a></h2>
            <p class="description">{{card.content}}</p>
            <p></p>
            {{ if {card.button_text} != '' }}  <a href="{{ if {card.button_link_external} }}{{card.button_link_external}}{{else-if {card.button_link_internal} }}{{truepath({card.button_link_internal})}}{{end-if}}"
            {{if {card.button_new_tab} }}target="_blank"{{end-if}} class="btn btn-info">{{card.button_text}}</a>{{end-if}}
          </div>
        </div>
        {{end-each}}
  ...
  <section id="about" class="about textured">
    <div class="container" data-aos="fade-up">
      ...
      <div class="section-title btf">
        <h3>{{this.below_the_fold_title}}</h3>
    ...
    <div class="container-fluid">
      ...
      <div class="row">
        ...
        <div class="col-lg-5 align-items-stretch video-box" style='background-image: url("{{this.faqs_image.getImage()}}");' data-aos="fade-right">
         {{if {this.faqs_video} }} <a href="{{this.faqs_video}}" class="venobox play-btn  mb-4" data-vbtype="video" data-autoplay="true"><span class="sr-only sr-only-focusable text-white">Play Video</span></a>{{end-if}}
        </div>

        <div class="col-lg-7 d-flex flex-column justify-content-center align-items-stretch" data-aos="fade-left">

          <div class="content">
            <h2>{{this.faqs_title}}</h2>
            {{this.faqs_content}}
          </div>
    ...
    <div class="container">
      ...
      <div class="row" data-aos="zoom-in">
        ...
        <div class="col-lg-3 cta-btn-container text-center">
          <a class="btn btn-info align-middle" href="{{if {this.button_link_internal} }}{{truepath({this.button_link_internal})}}{{else}}{{this.button_link_external}}{{end-if}}"{{if {this.cta_new_tab} }}target="_blank"{{end-if}}>{{this.cta_button_text}}</a>
        </div>
      </div>
      ...
      <div class="row">
        ...
        <div class="col-lg-6 mb-3" data-aos="fade-up" data-aos-delay="100">
                  <div class="card d-flex flex-row align-items-start justify-content-between">
                    <div class="pic align-self-center"><img src="{{t1.image.getImage(141,141,crop)}}" class="img-fluid" alt="{{t1.name}} Photo"></div>
                    <div class="card-info col-md-8 d-flex flex-column overflow-hidden">
                      <h4>{{t1.name}}</h4>
                      <span>{{t1.job_title}}</span>
                      <p>{{t1.bio_content.subWords(10)}}...</p>
                       {{if {t1.bio_content} }}
                      <p>{{t1.bio_content.subWords(10)}}...</p>
                      <a class="venobox text-info" data-vbtype="inline" href="#module{{t1._num}}">Read More</a>
                      {{end-if}}
                    </div>
                  </div>
                </div>
                <div id="module{{t1._num}}" class="" style="display:none;">
                  <div class="profile profile-smallimg">
                    <div class="profile__image">
                      <img src="{{t1.image.getImage(120,120,crop)}}" alt="{{t1.name}} Photo">
                    </div>
        ...
        <h3>{{each models as model}}{{if {model.name} == "team2" }}{{model.label}}{{end-if}}{{end-each}}</h3>
              ...
              <div class="row py-4">
                ...
                <div class="col-lg-4 mb-3 overflow-hidden" data-aos="fade-up" data-aos-delay="100">
                  <div class="card hz-mobile d-flex flex-column justify-content-between">
                    <div class="pic align-self-center"><img src="{{t2.image.getImage(141,141,crop)}}" class="img-fluid" alt="{{t2.name}}"></div>
                    <div class="card-info col-lg-12 col-md-8 d-flex flex-column">
                      <h4>{{t2.name}}</h4>
                      <span>{{t2.job_title}}</span>
                      {{if {t2.bio_content} }}
                      <p>{{t2.bio_content.subWords(10)}}...</p>
                      <a class="venobox text-info" data-vbtype="inline" href="#module_b{{t2._num}}">Read More</a>
                      {{end-if}}
                    </div>
                  </div>
                </div>
                <div id="module_b{{t2._num}}" class="" style="display:none;">
                  <div class="profile profile-smallimg">
                    <div class="profile__image">
                      <img src="{{t2.image.getImage()}}" alt="{{t2.name}}">
                    </div>
                    <div class="profile__info">
                      <div class="card-info">
                        <h4>{{t2.name}}</h4>
...
<div class="section-title">
  ...
  <section id="faq" class="faq light-bg">
    <div class="container" data-aos="fade-up">
